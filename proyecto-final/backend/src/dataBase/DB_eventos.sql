...
CREATE TABLE EventParticipants (
    idEventParticipant INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    userId INT NOT NULL,
    eventId INT NOT NULL,
    joinedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_event_participants_user FOREIGN KEY (userId) REFERENCES Users (idUser) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_event_participants_event FOREIGN KEY (eventId) REFERENCES Events (idEvent) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4;
...
CREATE TABLE EventHistory (
    idHistory INT NOT NULL AUTO_INCREMENT PRIMARY KEY, -- Identificador único
    eventId INT NOT NULL,                              -- FK a Events
    action VARCHAR(50) NOT NULL,                       -- 'created', 'updated', 'deleted', 'republished'
    performedBy INT NULL,                              -- FK a Users (admin/moderador)
    snapshot JSON NOT NULL,                            -- Estado del evento en ese momento
    timestamp DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_eventhistory_event FOREIGN KEY (eventId) REFERENCES Events (idEvent)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_eventhistory_user FOREIGN KEY (performedBy) REFERENCES Users (idUser)
